...
SCPN Stellarator Core — ADR 0003
-->
...
bind. Three candidates apply to the configurations this repository
owns: `closed.equilibrium_profiles` (noncyclic feature),
`closed.resolved_mhd_mode` (derived cyclic), and
`model.synthetic_oscillator_coordinate` (numerical only). The
`closed.recurrent_transient` candidate is catalogue-restricted to the
tokamak configurations and does not apply here.

## Decision

1. The capability `diagnostic_clock_semantics` is implemented as a
   declaration model (`src/scpn_stellarator_core/observability.py`):
   frozen `ClockModel`, `DiagnosticChannelPlan`, `DeferredCandidate`,
   and `DiagnosticPlan` objects with fail-closed invariants, an
...
3. Alignment is enforced structurally: a channel must address an
   ...
   (numerical-only to the simulation clock, derived cyclic to the
...
4. Timing is fail-closed where physics fixes the bound: the Nyquist
   criterion for cyclic channels. The device-typical band check is
   advisory only: magnetic-fluctuation diagnostics in the 0.5–200 kHz
   scale (I. H. Hutchinson, Principles of Plasma Diagnostics, 2nd ed.,
   2002, ch. 10), covering the low-frequency MHD activity relevant to
   heliotron, classical stellarator, and torsatron coil classes.
5. The reference plan mirrors canonical stellarator practice with
   synthetic declarations: a flux-loop equilibrium set, a line-density
   interferometer, Thomson-scattering profiles (all noncyclic against
   the plasma-initiation epoch), a Mirnov array (derived cyclic against
   the facility clock), and the model-owned synthetic oscillator
   (simulation clock). Every applicable candidate must be planned or
   explicitly deferred with a reason; every channel is
   `synthetic=True` by hard invariant. No SPO semantic-profile ingress
   is declared: the profile registry's `ingress_state` for these
   configurations stays `not_declared` until a real, versioned adapter
   exists.
