...
# SCPN Stellarator Core — diagnostic and clock semantics
...
identities for the stellarator configurations this repository owns,
aligned with the SCPN Phase Orchestrator observability-profile catalogue
release pinned by :data:`CATALOGUE_BINDING`. The applicable candidate
subset is embedded as data — this package never imports SCPN Phase
Orchestrator code. A plan declares HOW each evidence slot of a candidate
would be bound; it never claims that a diagnostic, a measurement, or a
facility exists. Every channel is synthetic by construction, review-only,
and non-actuating. Serialisation is canonical (sorted keys, no NaN or
...
from scpn_stellarator_core.configuration import ConsistencyFinding
        ...
        for configuration in self.configurations:
            if configuration not in OWNED_CONFIGURATIONS:
                raise DiagnosticPlanError(
                    ...
                    f"{configuration!r} is not owned by SCPN-STELLARATOR-CORE"
                )
...
APPLICABLE_CANDIDATES: Final = (
    CandidateProfile(
        ...
        phenomenon="magnetic equilibrium, geometry, and transport profiles",
        configurations=("heliotron", "stellarator", "torsatron"),
        observability_class=ObservabilityClass.NONCYCLIC_FEATURE,
    ),
    CandidateProfile(
        candidate_id="closed.resolved_mhd_mode",
        phenomenon="spatially and temporally resolved MHD mode",
        configurations=("heliotron", "stellarator", "torsatron"),
...
class DiagnosticChannelPlan:
    ...
    signals
        ...
        no ``timing_marker`` (no event-relative candidate is applicable),
        and a single ``phase``/``rad`` carrier for numerical-only channels.
    synthetic
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_id not in _CANDIDATE_INDEX:
            raise DiagnosticPlanError(
                ...
                "applicable to SCPN-STELLARATOR-CORE"
            )
        if not self.reason:
...
class DiagnosticPlan:
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            Findings from documented stellarator diagnostic practice;
            empty when every declaration sits in its typical band.
            Findings are advisory instruments, not machine claims.
        """
        findings: list[ConsistencyFinding] = []
        clocks_by_id = {clock.identifier: clock for clock in self.clocks}
        low, high = FLUCTUATION_BAND_HZ
        for channel in self.channels:
            if channel.candidate_id == "closed.resolved_mhd_mode" and not (
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            f"declared MHD band {channel.max_signal_frequency_hz:.1f}"
