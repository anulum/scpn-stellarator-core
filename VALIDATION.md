...
SCPN Stellarator Core — VALIDATION
-->
...
python3 agentic-shared/scripts/repository_tier0_scaffold_audit.py \
  03_CODE/SCPN-STELLARATOR-CORE --json
```
...
- Validated frozen parameter objects (`StellaratorGeometry`,
  `CoilRealisation`, `OperationalLimits`, `DeviceConfiguration`)
  rejecting non-finite values, bound violations, torus inversions, and
  identifier/coil-class contradictions — every rejection branch is
  tested.
- The hard coil-class invariant of the stellarator family: continuous
  helical windings for `heliotron`/`torsatron` (Uo, J. Phys. Soc. Japan
  16 (1961) 1380), modular non-planar coils for `stellarator` (Spitzer,
  Phys. Fluids 1 (1958) 253).
- Advisory consistency findings with documented bounds, reported and
  never clamped: aspect ratio below the classical stellarator regime
  (A < 3) and volume-averaged beta above routinely sustained values
  (⟨β⟩ > 5 %, LHD-class; Sakakibara et al., Nucl. Fusion 55 (2015)
  083020).
...
- A data-only pin (`ObservabilityBinding`) to the SPO
  ...
  release is rejected.
- A reference plan mirroring canonical stellarator practice with
  synthetic declarations: flux-loop equilibrium set, line-density
  interferometer, Thomson-scattering profiles, Mirnov array, and the
  model-owned synthetic oscillator, each bound to its clock domain.
- A documented advisory band check with its source stated in the code:
  magnetic-fluctuation diagnostics in the 0.5–200 kHz scale
  (Hutchinson, Principles of Plasma Diagnostics, 2nd ed., 2002,
  ch. 10); findings are reported, never clamped.
- Canonical serialisation (sorted keys, NaN/infinity rejected on both
  emit and parse), SHA-256 digest identity, and a strict round-trip
  parser that refuses unknown fields.
