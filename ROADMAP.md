...
1. **Device configuration model** — typed configuration policy for
   `stellarator`, `heliotron`, and `torsatron` (coil-set realisation
   classes, field-period and aspect-ratio envelopes, heating inventory),
   with evidence-maturity target `computational_prototype`.
