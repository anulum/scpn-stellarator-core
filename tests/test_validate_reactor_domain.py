...
# SCPN Stellarator Core — reactor-domain validator tests
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"configurations": ["heliotron", "heliotron"]},
            "must be unique",
        ),
        (
...
def test_map_cross_check_rejects_assignment_and_pin_drift(tmp_path: Path) -> None:
    ...
    map_path.write_text(
        json.dumps(
            {
                "planned_repositories": ["SCPN-STELLARATOR-CORE"],
                "existing_repositories": [],
                "configuration_assignments": {
                    "conventional_tokamak": "SCPN-STELLARATOR-CORE"
                },
