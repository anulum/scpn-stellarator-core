...
# SCPN Stellarator Core — repository-level truth tests
...
REQUIRED_PATHS = (
    ...
    "src/scpn_stellarator_core/__init__.py",
...
def test_manifest_declares_exact_configuration_assignment() -> None:
    ...
    assert manifest["project"] == "SCPN-STELLARATOR-CORE"
...
def test_package_agrees_with_manifest_truth() -> None:
    ...
    from scpn_stellarator_core import OWNED_CONFIGURATIONS, RegistryBinding
...
def test_typed_package_marker_exists() -> None:
    ...
    assert (REPO / "src" / "scpn_stellarator_core" / "py.typed").is_file()
